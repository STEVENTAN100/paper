...
新的点 \(x_p\) 被插入到 FreshVamana 索引中，使用算法 2。它查询当前索引中的点 \(p\) 的最近邻，以获取访问集 \(V\)。

![insert](./img/diskann/alg2-2.png)

通过在 \(V\) 上使用算法 3 的剪枝过程生成 \(x_p\) 的候选外部邻居，并在 \(p\) 和剪枝后的候选之间添加双向边。如果任何顶点的出度超过 \(R\)，则可以使用算法 3 将其剪枝至 \(R\)。
...
删除算法算法 4 参考了上文中的 **删除策略B**，关键特性是使用宽松的 \(\alpha\)-剪枝算法来保持修改后图的稠密性。具体而言，如果删除点 \(p\)，则在图中添加边 \( (p', p'') \)，其中 \( (p', p) \) 和 \( (p, p'') \) 是有向边。在此过程中，如果 \( |N_{out}(p')| \) 超过最大出度 \(R\)，我们使用算法 3 对其进行剪枝，以保持 \(\alpha\)-RNG 性质。

然而，由于该操作涉及编辑 \(p\) 的所有入邻居的邻域，代价可能很大，无法在删除到达时立即处理。FreshVamana 采用了懒删除策略 —— 当一个点 \(p\) 被删除时，我们将 \(p\) 添加到 DeleteList 中而不改变图。DeleteList 包含所有已删除但仍存在于图中的点。在搜索时，修改后的算法 1 使用 DeleteList 中的节点进行导航，但会从结果集中筛选掉它们。
